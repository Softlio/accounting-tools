...
export const ResultRow: React.FC<ResultRowProps> = ({
  ...
  useEffect(() => {
    ...
    const animation = async () => {
      await animate(
        scope.current,
        { opacity: 0.9, color: "var(--theme-secondary)" },
        { duration: 0.5 }
      );
      await animate(
        scope.current,
  ...
  let formattedValue =
    currentValue != undefined ? parseToEuro(value, negative) : "";
  ...
  return (
    <li className={cn("flex justify-between items-center", className)}>
      <span className="text-base md:text-lg max-md:flex-1">
        {label} {infoLabel && <InfoBubble {...infoLabel} title={label} />}
      </span>
      <span
        ref={scope}
        className={cn(
          "font-serif text-xl md:text-2xl whitespace-nowrap text-theme-primary overflow-hidden max-md:flex-1 max-md:text-right",
          valueClassName,
